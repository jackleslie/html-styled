...
<Paragraph>
  By adding a CSS selector name such as <Code inline>hover</Code> as a prop we
  get quick, direct access to selectors we want to use and don't have to write
  separate css or use bulky{" "}
...
<Code>{`<P color="red" hover={{ color: ["green", "blue"] }}>Hello world!</P>`}</Code>
